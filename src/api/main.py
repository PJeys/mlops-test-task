...
from src.utils.utils import load_config, load_json, load_pickle, get_timestamp_version
from src.api.schemas import SensorDataInput, PredictionResponse, HealthCheckResponse
...
async def load_model_and_preprocessor_on_startup():
    ...
    try:
        ...
        model_artifacts_dir = Path(CONFIG.get("model", {}).get("save_path", "artifacts/models"))

        if not model_artifacts_dir.exists():
            logger.error(f"Model artifacts directory not found: {model_artifacts_dir}")
            raise FileNotFoundError(f"Model artifacts directory not found: {model_artifacts_dir}")

        # Find the latest model based on timestamp in filename
        model_files = sorted(model_artifacts_dir.glob("*.joblib"), reverse=True)
        if not model_files:
            logger.warning(f"No trained models found in {model_artifacts_dir}. API will not be able to predict.")
        ...
        if not preprocessor_state_path.exists():
            logger.warning(f"Preprocessor state file not found at {preprocessor_state_path}. "
            ...
            for key, value in PREPROCESSOR_STATE.get("fit_params", {}).items():
                if isinstance(value, dict) and 'data' in value and 'columns' in value and 'index' in value:
                    try:
                        deserialized_fit_params[key] = pd.DataFrame(value['data'], index=value['index'],
                                                                    columns=value['columns'])
                    except Exception as e:
                        logger.warning(f"Could not deserialize DataFrame for fit_param '{key}': {e}. Using raw dict.")
        ...
        if not PREPROCESSOR.base_features:
            logger.warning("Preprocessor base_features not found in loaded state. "
            ...
            if MODEL.trained_features:
                ...
                logger.error("No features defined in preprocessor or model. Prediction will fail.")

        logger.info("Model and Preprocessor loaded successfully.")

    except Exception as e:
        logger.error(f"Failed to load model or preprocessor on startup: {e}", exc_info=True)
        # Optionally, raise an exception to prevent the app from starting if model loading is critical
        # raise RuntimeError(f"API startup failed: Could not load ML model components. {e}")
...
async def health_check():
    ...
    preprocessor_loaded = PREPROCESSOR is not None and PREPROCESSOR._fit_params and PREPROCESSOR.base_features
    ...
    return HealthCheckResponse(
        ...
        message="API ready for predictions." if model_loaded and preprocessor_loaded else "Model/Preprocessor not loaded. Check logs."
    )


@app.post("/predict", response_model=PredictionResponse, summary="Predict colony strength")
async def predict_colony_strength(data: SensorDataInput):
    ...
    try:
        ...
        # FastAPI's pydantic provides .dict() or .json() method
        ...
        logger.debug(f"Received input data for prediction: {input_df.to_dict(orient='records')}")

        # Preprocess input data using the loaded preprocessor
        # Ensure column order and types match expected by the preprocessor
        # The preprocessor.transform handles selecting and ordering features
        processed_input_df = PREPROCESSOR.transform(input_df)

        if processed_input_df.empty:
            logger.error("Preprocessing resulted in an empty DataFrame. Cannot predict.")
            raise HTTPException(
        ...
        # MODEL.classes_ stores the order of classes corresponding to probabilities
        prediction_probabilities = {str(cls): float(prob) for cls, prob in zip(MODEL.classes_, probabilities[0])}

        logger.info(f"Prediction made: {predicted_strength} with probabilities {prediction_probabilities}")
        ...
        logger.error(f"Data validation or feature mismatch error during prediction: {ve}", exc_info=True)
        raise HTTPException(
