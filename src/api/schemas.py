...
class SensorDataInput(BaseModel):
    ...
    timestamp: str = Field(
        ..., description="Timestamp of the sensor reading (ISO 8601 format)."
    )
    temperature_sensor: Optional[float] = Field(
        None, description="Temperature reading from the sensor."
    )
    firmware_version_sensor: Optional[str] = Field(
        None, description="Firmware version of the sensor."
    )
    gateway_id: Optional[str] = Field(None, description="Identifier for the gateway.")
    timestamp_gateway: Optional[str] = Field(
        None, description="Timestamp of the gateway reading (ISO 8601 format)."
    )
    ihs_to_gw_transmission_strength: Optional[float] = Field(
        None, description="Transmission strength between IHS and gateway."
    )
    firmware_version_gateway: Optional[str] = Field(
        None, description="Firmware version of the gateway."
    )
    temperature_gateway: Optional[float] = Field(
        None, description="Temperature reading from the gateway."
    )
    experiment_point_id: Optional[str] = Field(
        None, description="Identifier for the experiment point."
    )
...
class PredictionResponse(BaseModel):
    predicted_strength: str = Field(
        ..., description="The predicted colony strength (S, M, or L)."
    )
    prediction_probabilities: Dict[str, float] = Field(
        ..., description="Probabilities for each strength class."
    )
    model_version: str = Field(
        ..., description="Version of the model used for prediction."
    )
    message: str = Field(
        "Prediction successful.", description="A message regarding the prediction."
    )
...
class HealthCheckResponse(BaseModel):
    ...
    preprocessor_loaded: bool = Field(
        ..., description="Indicates if the preprocessor is loaded."
    )
